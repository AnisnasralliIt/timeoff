...
export function ApproveButton({ requestId }: { requestId: string }) {
  ...
  const approve = () => {
    startTransition(async () => {
      const state: ActionState = await approveLeaveRequestAction(requestId, {});
      if (state.ok) {
...
export function RejectRequestDialog({ requestId }: { requestId: string }) {
  ...
  const [state, formAction, pending] = React.useActionState<
    ActionState,
    FormData
  >(rejectLeaveRequestAction.bind(null, requestId), {});
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <form action={formAction} className="space-y-4">
          <Field label={t("reason")} required id="reject-reason">
